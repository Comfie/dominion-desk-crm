...
import { mandateService, updateRentalMandateSchema } from '@/lib/features/placement';
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validatedData = updateRentalMandateSchema.parse(body);
    const mandate = await mandateService.updateRentalMandate(userId, id, validatedData);
